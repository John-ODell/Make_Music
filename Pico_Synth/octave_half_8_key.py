...
# Setup additional buttons for frequency adjustment
half_note_button = Pin(27, Pin.IN, Pin.PULL_DOWN)  # Button to increase frequency by a half note
octave_button = Pin(28, Pin.IN, Pin.PULL_DOWN)     # Button to increase frequency by an octave
...
while True:
    ...
    for pin, key in keys.items():
        if key.value() == 1:
            frequency = notes[pin]
            if half_note_button.value() == 1:
                frequency *= 2 ** (1/12)  # Increase frequency by a half note
            if octave_button.value() == 1:
                frequency *= 2  # Increase frequency by an octave
            print(f"Key pressed: {pin}, Note: {frequency}")
            play_tone(buzzer1, buzzer2, frequency)
            key_pressed = True
            break
    if not key_pressed:
